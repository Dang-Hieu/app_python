...
def convert_image_to_hex_array(image_path):
    ...
    for y in range(height):
        for x in range(width):
            ...
            # Chuyển đổi sang phối màu Atkinson
            old_r, old_g, old_b = r, g, b
            r, g, b = (r // 32) * 32, (g // 32) * 32, (b // 64) * 64
            
            error_r, error_g, error_b = old_r - r, old_g - g, old_b - b

            # Áp dụng phương pháp Atkinson dựa trên điểm ảnh lỗi
            for i in range(2):
                for j in range(3):
                    if x + i < width and y + j < height:
                        img.putpixel((x + i, y + j), (
                            img.getpixel((x + i, y + j))[0] + error_r * 1 // 8,
                            img.getpixel((x + i, y + j))[1] + error_g * 1 // 8,
                            img.getpixel((x + i, y + j))[2] + error_b * 1 // 8
                        ))
